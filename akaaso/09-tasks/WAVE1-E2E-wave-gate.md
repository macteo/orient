# WAVE1-E2E-wave-gate — Wave 1 post-merge gate

Runs **inline on `develop`** after every impl task of wave 1 has merged.
...
Wave-specific: check:content green on the committed content (fixtures fail it); npm test green (logic + faces).

## Done Criteria

1. Every command above that applies to this wave exits 0 on `develop`.
2. No conflict marker anywhere in the tree.
3. The summary (commands run, counts, durations) is added to this task's history via `bun scripts/tasks/status.ts addHistory WAVE1-E2E-wave-gate gate "<summary>"`.
