# WAVE0-E2E-wave-gate — Wave 0 post-merge gate

Runs **inline on `develop`** after every impl task of wave 0 has merged.
...
Wave-specific: scaffold builds and tests; parsers/extractors ran and their outputs are committed; contact sheets reviewed.

## Done Criteria

1. Every command above that applies to this wave exits 0 on `develop`.
2. No conflict marker anywhere in the tree.
3. The summary (commands run, counts, durations) is added to this task's history via `bun scripts/tasks/status.ts addHistory WAVE0-E2E-wave-gate gate "<summary>"`.
