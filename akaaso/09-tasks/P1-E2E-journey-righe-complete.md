# P1-E2E-journey-righe-complete — @critical journey test for S-003

## Objective

Transcribe story S-003 (`akaaso/05-stories/003-righe-complete-in-entrambe-le-direzioni.md`) — its Journey Steps,
...
- `e2e/journeys/righe-complete.spec.ts` tagged `@critical`, one `test` per
...
- Seeding: empty storage unless the story says otherwise; the run seed via
  ...
  `akaaso/05-stories/fixtures/S-003/` copied to `e2e/fixtures/` when the
  folder exists.
...
- Register the test name **journey-righe-complete** in `e2e/README.md` (create if absent)
  with its story id.

### Pages crossed

home, flashcard, quiz, risultati. Features exercised: F-001, F-002, F-003, F-004, F-006, F-008.
...
2. Every Journey Step of S-003 corresponds to a step in the spec, in order,
...
- Depends on every impl task of the features above. Story: S-003 (spine).
