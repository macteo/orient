# WAVE3-E2E-wave-gate — Wave 3 post-merge gate

Runs **inline on `develop`** after every impl task of wave 3 has merged.
...
Wave-specific: build emits thirteen pages; e2e:smoke loads every page; check:dist passes.

## Done Criteria

1. Every command above that applies to this wave exits 0 on `develop`.
2. No conflict marker anywhere in the tree.
3. The summary (commands run, counts, durations) is added to this task's history via `bun scripts/tasks/status.ts addHistory WAVE3-E2E-wave-gate gate "<summary>"`.
