# WAVE5-E2E-wave-gate — Wave 5 post-merge gate

Runs **inline on `develop`** after every impl task of wave 5 has merged.
...
Wave-specific: workflow green on develop→main; @critical journeys green under /orient/ and /; site answers at the Pages URL.

## Done Criteria

1. Every command above that applies to this wave exits 0 on `develop`.
2. No conflict marker anywhere in the tree.
3. The summary (commands run, counts, durations) is added to this task's history via `bun scripts/tasks/status.ts addHistory WAVE5-E2E-wave-gate gate "<summary>"`.
