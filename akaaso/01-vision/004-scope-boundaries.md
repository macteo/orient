...
- **Four decks from two source documents** (widened on 2026-09-05 at the
  founder's request; the original two-deck bullet follows, the two added
  decks are defined at Level 3 [3.004](../03-modules/004-pipeline-descrizioni-righe.md)
  and [3.005](../03-modules/005-pipeline-esempi.md)):
  - *Descrizioni complete* — a whole description row, columns C – H, with
    its Italian sentence; nine official rows and rows generated by rule.
  - *Esempi sul terreno* — the worked examples of [S3]: map clip, terrain
    sketch, printed row, sentence.
...
- **Results kept in the learner's browser.** Every completed run (deck,
  sections, mode, date, score, the cards missed) and the run in progress
  are stored on the device, in the browser's local storage, and shown
  back: last score per deck on the home, a results page per deck with the
  misses to review. Nothing leaves the device; nothing identifies a
  person; a "cancella i risultati" action wipes it. Added on 2026-09-05 at
  the founder's request ("tieni i risultati sul client").
...
- Progress *across* devices, statistics beyond the stored runs, spaced
  repetition. The per-browser results above are in; anything that needs
  the results to leave the device is out.
...
- 2026-09-05 (later) — results in the browser moved into scope at the founder's request. Dependent decisions reviewed: 1.005 (no personal data — holds, storage is on-device and anonymous), 2.003, 2.007, 2.008, 3.002, 3.008 amended the same day.
- 2026-09-05 (later) — widened from two decks to four at the founder's request when opening Level 3: full description rows and worked examples added, both from [S3]. Still two documents, still verbatim text (generated rows are marked as such). 1.003 criterion 3 unchanged; completeness of the new decks is defined in 3.004 and 3.005.
